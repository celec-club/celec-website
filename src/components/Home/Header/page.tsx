...
export default function Header() {
    ...
    useEffect(() => {
        const checkDeadline = () => {
            ...
            if (diffDays > 0) {
                setTimeRemaining(`${diffDays}d ${diffHours}h left`);
            } else if (diffHours > 0) {
                setTimeRemaining(`${diffHours}h ${diffMinutes}m left`);
            } else {
                setTimeRemaining(`${diffMinutes}m left`);
            }
    ...
    // Show popup when registration is open and user hovers over join button
    useEffect(() => {
    ...
    const handleJoinClick = (e: React.MouseEvent) => {
        ...
        // Allow default behavior if registration is open
    };
    ...
    const JoinButton = ({ isMobile = false }: { isMobile?: boolean }) => (
        <div className="relative">
            ...
            {/* Popup for registration deadline */}
            <AnimatePresence>
                {showDeadlinePopup && !isMobile && (
                    <motion.div
                        ...
                        className="absolute top-full right-0 mt-2 w-64 p-4 bg-gray-800/95 backdrop-blur-sm rounded-lg shadow-xl border border-gray-700 z-50"
                    >
                        {/* Close button */}
                        <button
                        ...
                        <div className="flex items-start gap-3">
                            ...
                            <div className="flex-1">
                                ...
                                <p className="text-sm text-gray-300 mb-2">
                                    Join before: <span className="text-blue-300 font-medium">
            ...
            {/* Static tooltip for closed registration (doesn't auto-hide) */}
            {(!isRegistrationOpen && isHovering && !isMobile) && (
                <div className="absolute top-full right-0 mt-2 w-64 p-3 bg-gray-800/95 backdrop-blur-sm rounded-lg shadow-lg border border-gray-700 z-50">
                    <div className="flex items-start gap-3">
                        <Calendar size={16} className="text-gray-400 mt-0.5" />
                        <div>
                            <p className="font-medium text-white">Registration Closed</p>
        ...
        >
            <div className="max-w-7xl mx-auto px-6">
                <div className={`flex items-center justify-between transition-all duration-300 ${headerShrink ? "h-16" : "h-14"
                    ...
                    {/* Right: Join Button and Mobile Menu */}
            ...
            {isOpen && (
                <motion.div
                    ...
                    className="md:hidden backdrop-blur-lg bg-[#0A0F2D]/95 w-full px-4 py-6 flex flex-col items-center gap-6 border-t border-blue-900/30"
                >
                    {navLinks.map((link) => (
                        <a
                            key={link.name}
